...
export const useAvailableSlots = (departmentId: string, date: string, doctorId?: string) => {
    return useQuery({
        queryKey: ['slots', departmentId, date, doctorId],
        queryFn: () => appointmentApi.getAvailableSlots(departmentId, date, doctorId),
...
export const useSaveDoctorPlan = () => {
    ...
    return useMutation({
        mutationFn: ({ appointmentId, planData }: { appointmentId: string, planData: any }) => 
